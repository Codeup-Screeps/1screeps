...
class PathingManager {
  ...
  moveTo(creep, target, defaultOptions = {}) {
    ...
    if (instance.spawning) {
      Utils.logError(
        new Error(
          `Pathfinder: can't move creep that is spawning '${creep.name}'`
        )
      );
      return ERR_INVALID_ARGS;
    }
    ...
    if (!fatigue) {
      ...
      if (
        path.length === 0 ||
        (pathIncomplete && (!allowIncomplete || path.length <= 1))
      ) {
        return ERR_NO_PATH;
      }
  ...
  moveOffRoad(creep, options = {}) {
    ...
    if (matrix.get(creepPos.x, creepPos.y) !== 1) {
      // not on road
      return false;
    }
  ...
  moveCreeps(moves) {
    for (let i = 0; i < moves.length; i++) {
      ...
      if (offRoad) {
        ...
        const offRoadPos = this.getCreepOffRoadMovePos(
          instance,
          offRoad,
          priority,
          moves,
      ...
      if (blocked || pushed || offRoad) {
        const obstacleInstance = this.getObstacleCreep(
          move.pos || (move.pos = Utils.offsetPosCoords(creepPos, direction)),
          instance.room
        );
        if (obstacleInstance) {
          const obstacleCreep =
            obstacleInstance.my && this.isCreepCanMove(obstacleInstance)
          ...
          if (!obstacleCreep) {
            ...
            const movePos = this.getCreepMovePos(
              instance,
              priority,
              moves,
              pathEnd
            ...
            direction = move.direction = movePos
              ? Utils.getDirection(creepPos, movePos)
              : 0;
          } else if (
            !obstacleInstance.fatigue &&
            (obstacleCreep._moveTime !== Game.time ||
              obstacleCreep._offRoadTime === Game.time)
          ) {
            ...
            const targetInfo = this.getCreepTargetInfo(
              obstacleCreep,
              obstacleInstance.room.name
            );
            let moveDirection, movePos;
            if (
              targetInfo ||
              pushed ||
              this.hasMove(creepPos, moves, priority)
            ) {
              // determine blocking creep move direction
              const pushPos = this.getCreepPushPos(
                obstacleInstance,
                preferOffRoad,
                priority,
                moves,
              ...
              moveDirection = pushPos
                ? Utils.getDirection(obstacleCreepPos, pushPos)
                : 0;
            } else {
              // swap positions
  ...
  getObstacleCreep(pos, room) {
    ...
    if (!creep) {
      ...
      creep =
        lookPowerCreeps.find((creep) => creep.my) ||
        lookCreeps[0] ||
        lookPowerCreeps[0];
    }
    return creep;
  ...
  hasObstacleCreep(room, x, y) {
    return (
      room
        ...
        .find(
          (creep) =>
            !creep.my || creep.fatigue > 0 || !this.isCreepCanMove(creep)
  ...
  *getAdjacentPositions(pos) {
  ...
  getCreepOffRoadMovePos(creep, offRoad, priority, moves, targetInfo) {
    ...
    for (const pos of this.getAdjacentPositions(creepPos)) {
      ...
      if (
        cost === 255 ||
        cost === 1 ||
        this.hasMoveWithOffRoad(pos, moves, priority)
      ) {
        continue;
      }
      if (this.hasObstacleCreep(room, pos.x, pos.y)) {
        ...
        if (
          targetInfo &&
          Utils.getRange(pos, targetInfo.pos) > targetInfo.range
        ) {
          continue;
        }
  ...
  findPath(startPos, targetPos, defaultOptions = {}) {
    ...
    if (
      ...
      const route = this.findRoute(startRoomName, targetRoomName, {
        avoidRooms,
        routeCallback,
      });
      if (route.length > 0) {
        ...
        console.log(
          `Pathfinder: Could not find route from ${startRoomName} to ${targetRoomName}. source: ${startPos} target: ${targetPos}`
        );
      }
    }
  ...
  getTargetRangePositions(targetPos, range, options) {
    ...
    for (const pos of positions) {
      const cost =
        (matrix && matrix.get(pos.x, pos.y)) || terrain.getCost(pos.x, pos.y);
      if (cost < 255) {
  ...
  fixPath(path, matrix, roomName) {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      let cost = matrix.get(pos.x, pos.y) || terrain.getCost(pos.x, pos.y);
      if (i >= 2 && cost === 1 && costs[i - 1] !== 1 && costs[i - 2] === 1) {
    ...
    if (
      ...
      room.find(FIND_STRUCTURES).forEach((structure) => {
        ...
        if (structure.structureType === STRUCTURE_ROAD) {
          ...
          if (!ignoreTunnels && cell & TERRAIN_MASK_WALL) {
            ...
            (!ignoreRoads || (swampRoads && cell & TERRAIN_MASK_SWAMP)) &&
            matrix.get(x, y) === 0
          ) {
      ...
      if (!ignoreStructures) {
        room.find(FIND_MY_CONSTRUCTION_SITES).forEach((construction) => {
          if (
            ![STRUCTURE_CONTAINER, STRUCTURE_ROAD, STRUCTURE_RAMPART].includes(
